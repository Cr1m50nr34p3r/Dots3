...
parser.add_argument(
    "-c",
    default=False,
    help="print logs of a given date by default checks for current date use with -d to change date",
    type=bool,
)
parser.add_argument(
    "-r",
    default=False,
    help="Read logs of a given date and print number of args use with -n arg by default checks for current date",
    type=bool,
)
    ...
    while running:
        try:
            ...
            td = datetime.timedelta(
                hours=dur.hour, minutes=dur.minute, seconds=dur.second
            )
            end_time = now + td
